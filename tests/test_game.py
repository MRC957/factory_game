...
class TestDashboardHelpers:
    def test_dashboard_status_contains_key_info(self):
        g = FactoryGame(seed=0)
        status = g.dashboard_status()
        assert "Day 1" in status
        assert "Cash" in status
        assert "Workers" in status
        assert "Inventory" in status

    def test_dashboard_status_shows_owned_blueprints(self):
        g = _game_with_cash(1000.0)
        g.buy_blueprint("smelter_optimization")
        status = g.dashboard_status()
        assert "smelter_optimization" in status

    def test_dashboard_status_no_blueprints_shows_none(self):
        g = FactoryGame(seed=0)
        status = g.dashboard_status()
        assert "none" in status

    def test_dashboard_market_lists_all_items(self):
        g = FactoryGame(seed=0)
        market = g.dashboard_market()
        assert "Market prices:" in market
        for item in g.market_prices:
            assert item in market

    def test_dashboard_market_shows_arrows(self):
        g = FactoryGame(seed=1)
        g.next_day()  # triggers price changes
        market = g.dashboard_market()
        assert any(arrow in market for arrow in ["↑", "↓", "→"])

    def test_dashboard_margins_lists_all_recipes(self):
        g = FactoryGame(seed=0)
        margins = g.dashboard_margins()
        assert "Estimated recipe margins" in margins
        for recipe in g.recipes:
            assert recipe in margins

    def test_dashboard_margins_shows_verdict(self):
        g = FactoryGame(seed=0)
        margins = g.dashboard_margins()
        assert any(v in margins for v in ["profitable", "unprofitable", "risky"])

    def test_help_text_lists_commands(self):
        g = FactoryGame(seed=0)
        help_text = g.help_text()
        for cmd in ["buy", "sell", "craft", "hire", "fire", "assign", "next", "quit"]:
            assert cmd in help_text

    def test_blueprint_text_lists_all_blueprints(self):
        g = FactoryGame(seed=0)
        text = g.blueprint_text()
        assert "Blueprint shop:" in text
        for name in g.blueprints:
            assert name in text

    def test_blueprint_text_marks_owned(self):
        g = _game_with_cash(1000.0)
        g.buy_blueprint("smelter_optimization")
        text = g.blueprint_text()
